...
class AudioPlayer():
    ...
    def load_audio_file(self, audio_file_idx, start_time=0):
        ...
        self.playback = Playback(filename=self.audio_filenames[self.current_audio_idx], callback=self.audio_callback, ff_opts={'sync': 'ext', 'ss': start_time, 'vn': True, 'af': 'atempo=1.0'})
        if self.playing:
